...
缺失的 sheet3 期外票），弹一次提示并可一跳直达「发票补录」页。补录入口**按模式分**
（阶段 3 B2g）：`pre`（导入前）本页行内「补录原票」按钮可用，填好后随本次台账
**同一事务**入库；`post`（导入后）该按钮隐藏（只读模式不给入口），补录一律走
「发票补录」页（立即写库）。
...
class ImportReviewView(QWidget):
    ...
    def _prompt_backfill(self) -> bool:
        ...
        box.setInformativeText(
            ...
            "「补录原票」入口有两种：**导入前**用本页行内的「补录原票」按钮（填好后"
            "随本次台账一起入库），**导入后**该按钮已隐藏、须走「发票补录」页；"
            "两类来源都已按台账已有信息预填，逐张核对后保存即可。补录完成后回本页，"
            "该行会转为普通比对行。")
        box.setDetailedText("\n".join(
