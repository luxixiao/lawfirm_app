...
### 4-2 复核页：三分类 + 回显 + 文案（⏳ 待实施）

| 文件 | 改动点 |
|---|---|
| `app/ui/unified_import_dialog.py` | `FILTERS` 插入 `待补录`（**排在 `待确认` 之前**）；`_PRIO` = 待补录 0 / 待确认 1 / 高置信 2 / 已跳过 3；**默认筛选 = 待补录** |
| 同上 | deferred 行状态改三态：需补录**且未填补录** → **待补录**；已填补录**未确认** → **待确认**；确认后 → 高置信 |
| 同上 | `_open_backfill` **不再**把该行塞进 `_deferred_confirmed`（这正是「补录完直接变高置信」的根因）；改为把补录里填的经办人/收款**写回该行** → 待确认里**回显**（右栏预填 + 「各经办人已收」列） |
| 同上 | `_confirm_row`（已补录行）= 确认收款信息 → 归入已确认；`accept()` 的「未处理行」统计纳入**待补录**行 |
| 同上 | 按钮文案三态：未补录 → `补录原票`；**已填补录 → `修改补录`**；已在库 → `查看原票` |
| 同上 | 右栏原因三态：`需补录原票`（琥珀）/ `已补录，请确认收款`（蓝）/ `已入库，请确认收款`（蓝）；底部汇总同步拆计数 |
