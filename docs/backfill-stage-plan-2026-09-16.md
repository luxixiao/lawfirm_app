...
- **B2h 已知交互（阶段 4-2 起已改，原文作废）**：~~行内补录成功后该行**归入已确认**
  （`_deferred_confirmed`）→ 离开「待确认」筛选~~。现在补录成功后该行落到**「待确认」**
  （原因 `已补录，请确认收款`、按钮翻 `修改补录`），默认筛选是**「待补录」**，故该行在默认
  视图里同样看不到 → 断言/操作前先切「全部」。点「确认」后才归入已确认、离开待确认。
  底部汇总给出「已补录待确认收款 N 张」与「本次已填补录 N 张（随本次台账入库）」两个计数。
...
## 4A. 阶段 4：补录后的二次确认（待补录 → 待确认 → 确认收款）

**背景（用户 2026-09-17 提出）**：行内补录一完成，该行立刻变「高置信」直接归入已确认 ——
少了「补录完再看一眼收款信息」这一步。要求：分类里再加一个**「待补录」**，
补录完成后落到**「待确认」**，在那里确认收款信息。

**用户拍板（A甲 / B甲 / C甲 / D甲）**：

| # | 决策点 | 取值 |
|---|---|---|
| A | 待补录里能否填收款信息 | **能填**。红冲导致的补录**必须**有收款（台账行没有收款来源）；sheet3 导致的补录有两个入口（复核页行内按钮 / 「发票补录」页）。**在待补录里填过的收款，要在待确认里回显** |
| B | 默认筛选 | **待补录** |
| C | 「待补录」在筛选栏的位置 | **排在「待确认」之前**（最紧急、排最前） |
| D | 补录完成后的按钮文案 | **「修改补录」**（点它确实是重新打开可编辑，而不是「查看原票」） |

**批次**：4-1 引擎（写序 + 去重）→ 4-2 复核页（分类 / 回显 / 文案）。

### 4-1 引擎：A10 写序对齐 + 补录去重（✅ 已落地）

写入顺序统一为：**sheet3 镜表 → 普通票 → 补录票 → A10 确认收款**。

1. **写序**：A10（复核页确认收款 → 追加 collection）原排在 `_write_backfills` **之前**。
   它下面那句「票不在库就跳过」原本是为**补录票**写的兜底，可 A10 跑的时候补录票还没建，
   于是把本批刚补录出来的票一并跳过 → 该票的「确认收款」永远写不进去。挪到补录之后即可。
2. **去重**：A10 跳过**本批 `backfills` 覆盖的票号**。补录写 `source='manual'`
   （`apply_backfill`）、A10 写 `source='import'`（`_append_receipts_to_existing`）；
   同一票号两边都写 → 同一笔收款 import + manual 各记一份 → 全来源合计翻倍 →
   被超额校验判成「台账信息错误」（**假报错**、整行拒写）。
   口径：**补录是显式指令，优先于 A10 的台账兜底确认**。
   落地形态：`bf_nos = {票号} - {""}`；A10 循环内 `if no in bf_nos: continue`。

> **⚠️ 如实说明（不要当成"已修 bug"）**：这**两条在本流程下都走不到** ——
> 复核页产出的补录票一律是 `need_backfill=True`，`_merged_data` 对这类行
> **明确不出收款**（`if d.get("need_backfill"): continue`），因此它永远不会带
> `receipt_confirmed`，A10 根本不会碰它。所以 4-1 是**潜伏缺陷的预防**（写序读起来自洽 +
> 去重守卫兜住"未来把补录票也纳入 A10"的情形），**不是**在当前 UI 下能复现的故障。
> 之所以现在做：成本极低（<20 行 + 一组对照测试），而一旦踩到就是"静默重复计 + 假报错"。

**测试**：`tests/test_backfill_pending.py` **39 → 45**（新增 H 段 6 项）：
同票号收款只写一套（manual）/ 无 import 重复 / 不进 `over_collected` /
**真·已在库票的 A10 照常追加**（证明 A10 仍在跑、去重只限补录票号）/
两套相加本会超额也**不产生假报错** / 该票仍只有一套。
**反向校验**：临时关掉去重守卫重跑 → H1/H2/H5 如实变红（证明这组测试不是空转）。

### 4-2 复核页：四态 + 回显 + 文案（✅ 已实施）

| 文件 | 改动点 |
|---|---|
| `app/ui/unified_import_dialog.py` | `FILTERS` 插入 `待补录`（**排在 `待确认` 之前**，C 甲）；`_PRIO` = 待补录 0 / 待确认 1 / 高置信 2 / 已跳过 3；**默认筛选 = 待补录**（B 甲） |
| 同上 | 新增 `_deferred_status(d, d_index)`：已确认 → `高置信`；**需补录且本次未填补录 → `待补录`**；其余（已在库 / 已填补录）→ `待确认` |
| 同上 | `_open_backfill` **不再**把该行塞进 `_deferred_confirmed`（这正是「补录完直接变高置信」的根因）；改为把补录里填的经办人/收款**写回该行**（`_deferred_edits`，新增 `_row_edit_from_backfill`）→ 该行落到**待确认**并在右栏预填 + 「各经办人已收」列**回显**（A 甲） |
| 同上 | `_confirm_row`：`待补录` 行点「确认」→ `QMessageBox` 可读提示（「需先补录原票…本次点「确认」不会写入任何数据」）并**直接 return**（不写任何东西）；`待确认` 行才 `_deferred_confirmed.add` |
| 同上 | `_load_right`：deferred 行 `confirm=(status != "待补录")` → 待补录只给「保存修改」，不给「确认」 |
| 同上 | 按钮文案三态：未补录 → `补录原票`；**已填补录 → `修改补录`**（D 甲）；已在库 → `查看原票` |
| 同上 | 原因列：`需补录原票`（琥珀）/ **`已补录，请确认收款`（蓝）** / `已入库，请确认收款`（蓝）；常量 `REASON_BACKFILLED` 取代旧 `REASON_BACKFILL_DONE` |
| 同上 | 底部统计胶囊 `待补录 N　待确认 N　高置信 N`；汇总拆为 `需补录原票` / `已补录待确认收款` / `已入库待确认收款` / `本次已填补录` |
| 同上 | `_merged_data()` 在 `bf_map.update(...)` 之后，用 deferred 行**最终值**回写 `backfills` 条目（`_bf_handlers_from_row`）→ 在待确认里改过的收款仍随补录落库 |
| 同上 | `accept()` 的「未处理行」统计纳入 `待补录` 行 |

> 4-2 落地后，上文 B2h 的既有语义（**「补录成功后该行离开待确认、直接变高置信」**）
> **作废** —— 改为：补录后落在**待确认**，确认收款信息之后才离开。
> 全链路：**待补录**（点「补录原票」填经办人/收款）→ **待确认**（回显，核对后点「确认」）→ **高置信**。

**测试**：`tests/_smoke_unified_import.py` **152/152 通过**（新增 6 条「4-2 / A 甲」断言：
状态四态与排序 / 默认筛选 = 待补录且默认勾选 id 0 / 筛选顺序 / 统计胶囊 /
补录后落到待确认（不再高置信）/ 按钮翻 `修改补录` / 原因 `已补录，请确认收款` /
待补录行不给「确认」按钮 / **待补录里填的收款 → 待确认回显**（右栏预填 + 各经办人已收列 +
随 `backfills` 落库）/ 回显后仍不置 `receipt_confirmed`）。
**反向校验**：临时把 `_open_backfill` 退回旧行为（`_deferred_confirmed.add`）→
4 条断言如实变红（`落到待确认` / `已补录待确认收款` / `尚未确认` / `A 甲回显`），
证明这组断言不是空转。

**其它测试适配（同样受"默认筛选变了"影响）**：
`tests/_smoke_font_scale.py` 复核页表格行高两只断言（34/39）需先切「全部」（**30/30**）；
`tests/_smoke_unified_import.py` 第 6/9/10 节的选行统一改用
`_show_all()` + `_select()`（`setCurrentCell` + `selectRow` + `_load_right`）。
全量回归 **29/29 全绿**（40.5s）。
...
| **阶段3 右栏原因（第 4 态，4-2 改文案）** | `已补录，请确认收款`（`unified_import_dialog.REASON_BACKFILLED`；填过补录后显示，**蓝色** —— 它属于「待确认」而非「已完成」） |
| **阶段3 底部汇总（4-2 改）** | `·　已补录待确认收款 {n} 张　·　本次已填补录 {n} 张（随本次台账入库）` |
| **B2e 弹窗内拦截** | `票号 {no} 本次已填过补录，请只保留一份（如需修改，请点该行的「修改补录」）。` |
